...
local HTTP target을 시작하고, temporary `scanrail.yaml`을 생성하고, 현재 Scanrail binary를 build한 뒤 stdio MCP server로 노출합니다. test spec은 discovery, resource, safety gating, confirmed native headers scan, latest report summary retrieval을 검증합니다.
