...
Run the default quick profile, which checks local secrets with Gitleaks, web response headers and TLS posture when a target is configured, and an OpenAPI contract when a local spec is configured:
...
Use `scanrail run --only headers`, `scanrail run --only tls`, or `scanrail run --only openapi` when Docker is unavailable, or `scanrail run --only gitleaks` to run only the Docker-backed secrets scan.
