...
이 문서는 첫 public npm release, trusted publishing 설정, `0.1.2` MCP MVP, MCP audit logging, MCP Workbench 검증, `0.1.4` Gitleaks adapter slice, `0.2.0` native TLS baseline slice, `0.2.1` native OpenAPI baseline slice, `0.2.2` target guardrail slice 이후 남은 주요 리스크를 추적합니다.
...
| scanner adapter | native headers, native TLS baseline, native local OpenAPI baseline, web target guardrail preflight, Gitleaks용 첫 Docker-backed adapter 구현 | Trivy/Semgrep 실행, broader DAST, third-party container egress enforcement, OS matrix coverage, digest-level image policy는 계획 단계 |
...
| R-006 | MCP tool이 core policy와 diverge하면 CLI safety를 우회할 수 있음 | High | 설계로 통제 | MCP는 CLI safety model 위의 thin adapter로 유지하고, active scan은 `confirm_active_scan=true`를 요구하며 MCP headers scan은 shared allowlist/path guardrail을 재사용합니다. | MCP tool implementation이 CLI execution과 같은 config, exit-code, safety validation path를 사용하게 유지합니다. |
...
| R-009 | scanner adapter 확장으로 network 또는 credential exposure 증가 | High | 부분 완화 | Gitleaks는 read-only workspace mount와 writable raw output mount를 사용하는 passive local scanner로 활성화됐습니다. 준비되지 않은 Docker adapter는 profile 실행에서 skip되고, 사용자가 명시 선택하면 exit code `5`로 실패합니다. native headers scanner는 interactive network capability를 선언하고 redirect를 따르지 않습니다. native TLS scanner는 단일 TLS connection만 열고 HTTP payload를 보내지 않으며 certificate/protocol finding만 리포트합니다. native OpenAPI scanner는 local spec file만 읽고 URL 형태의 spec을 거부합니다. | Trivy, Semgrep 또는 broader networked Docker scanner를 활성화하기 전 adapter-specific integration coverage를 추가합니다. |
...
| R-014 | 사용자가 현재 headers, TLS, OpenAPI, Gitleaks scan의 coverage를 과대평가할 수 있음 | Medium | 문서 리스크 | README와 docs는 MVP가 commercial scanner parity가 아니며 Trivy, Semgrep, SARIF, ZAP/Nuclei, active OpenAPI scans, broader DAST가 planned 상태라고 설명합니다. | README, report, release note에 capability와 non-goal 문구를 계속 노출합니다. |
