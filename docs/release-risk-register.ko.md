...
이 문서는 첫 public npm release, trusted publishing 설정, `0.1.2` MCP MVP, MCP audit logging, MCP Workbench 검증, `0.1.4` Gitleaks adapter slice, `0.2.0` native TLS baseline slice, `0.2.1` native OpenAPI baseline slice 이후 남은 주요 리스크를 추적합니다.
...
| scanner adapter | native headers, native TLS baseline, native local OpenAPI baseline, Gitleaks용 첫 Docker-backed adapter 구현 | Trivy/Semgrep 실행, broader DAST, OS matrix coverage, digest-level image policy는 계획 단계 |
