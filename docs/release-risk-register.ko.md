...
이 문서는 첫 public npm release, trusted publishing 설정, `0.1.2` MCP MVP, MCP audit logging, MCP Workbench 검증, `0.1.4` Gitleaks adapter slice 이후 남은 주요 리스크를 추적합니다.
...
| scanner adapter | Gitleaks용 첫 Docker-backed adapter 구현 | Trivy/Semgrep 실행, OS matrix coverage, digest-level image policy는 계획 단계 |
| reporting | headers와 Gitleaks용 JSON/HTML 제공 | SARIF와 false-positive workflow는 계획 단계 |
...
| R-014 | 사용자가 현재 headers + Gitleaks scan의 coverage를 과대평가할 수 있음 | Medium | 문서 리스크 | README와 docs는 MVP가 commercial scanner parity가 아니며 Trivy, Semgrep, SARIF, broader DAST가 planned 상태라고 설명합니다. | README, report, release note에 capability와 non-goal 문구를 계속 노출합니다. |
