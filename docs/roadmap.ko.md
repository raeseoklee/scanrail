...
- web target allowlist와 blocked path guardrail (`0.2.2` shipped)
- ZAP OpenAPI scan
- OWASP ZAP baseline
...
- testssl.sh
- bearer/cookie 인증
- allowlist 검증 (`0.2.2`에서 native interactive scanner 대상 shipped)
- blocked paths (`0.2.2`에서 native interactive scanner 대상 shipped)
- rate limit
...
상태 메모: `0.2.0`은 native TLS baseline을, `0.2.1`은 local-file-only native OpenAPI baseline을, `0.2.2`는 native interactive scanner용 configured web target guardrail enforcement를 제공합니다. ZAP, Nuclei, Schemathesis, testssl.sh, active OpenAPI scan은 이 milestone의 planned 항목으로 남아 있습니다.
