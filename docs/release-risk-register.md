...
| R-014 | Users may overestimate coverage from the current headers, TLS, OpenAPI, and Gitleaks scan | Medium | Documentation risk | README and docs state that the MVP is not commercial scanner parity and that Trivy, Semgrep, SARIF, ZAP/Nuclei, active OpenAPI scans, and broader DAST remain planned. | Keep capability and non-goal language visible in README, reports, and release notes. |
