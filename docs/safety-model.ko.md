...
native TLS scanner는 네트워크 연결을 만들기 때문에 `interactive`로 분류하지만, 현재 구현은 단일 TLS handshake만 수행합니다. native OpenAPI scanner는 local file만 읽으므로 `passive`입니다. Schemathesis, ZAP API scan, intrusive Nuclei templates는 profile 이름과 무관하게 최소 `interactive`로 취급합니다.
